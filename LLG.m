...
denomnum =  ( Z12(faultBus,faultBus) * (Z0(faultBus,faultBus)+3*Zl) );
denomdenom = Z12(faultBus,faultBus) + Z0(faultBus,faultBus) + 3*Zl;

denom = Z12(faultBus,faultBus) + (denomnum/denomdenom  );



If1 = 1 / denom;

If2 = -1*If1 * ( Z0(faultBus,faultBus) + 3*Zl ) / (denomdenom);
If0 = -1*If1 * ( Z12(faultBus,faultBus) ) / (denomdenom);
...
fprintf(fileID,'#########################################################################################\n')
fprintf(fileID,'-----------------------------------------------------------------------------------------\n');
fprintf(fileID,'                              LLG Fault Analysis \n');
fprintf(fileID,'-----------------------------------------------------------------------------------------\n');
